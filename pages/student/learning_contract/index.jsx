...
//import { createLearningContract } from '../../../api/learning_contract_api';
...
import { notifyAdminsAboutLearningContractUpload } from '../../../api/notifications_api';
import { createOrUpdateLearningContract } from '../../../api/learning_contract_api';

function StudentLearningContractUpload({ user, token }) {
	...
	if ((token && user) && (user.id && user?.role == 'student')) {
		return (
			<center>
				...
				<h1>Upload Learning Contract </h1>
				<br></br>
							...
							if (document.getElementById('docName') && document.getElementById('docName').value) {
								...
								if (name.length >= 3 && name.length <= 60) {
									if (checkFileName(name)) {
										if (confirm('Are you sure you want to share this document with any administrator from UTA?')
											&& document.getElementById('docName'))
										{
											...
											reader.addEventListener("load", async () => {
												...
												//const success = await createLearningContract(token, user.id, name, (reader.result).toString('binary'));
												const success = await createOrUpdateLearningContract(token, user.id, name, (reader.result).toString('binary'));
												const notifySuccess = await notifyAdminsAboutLearningContractUpload( token, user.id);
...
export default StudentLearningContractUpload;

StudentLearningContractUpload.getInitialProps = async ({ req }) => {

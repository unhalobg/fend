...
import { adminRetrieveAllPlacementRequests } from '../../../api/agency_student_request_application_api'
...
/**
 * @param {Array.<Object>} formList array of placement requests from agencies
 */
    ...
    if (formList.length > 0) {
        return (
            <Grid container justifyContent={"center"} style={{ marginTop: "3%" }}>
                <Grid container justifyContent={"left"} style={{ width: "70%" }}>
                    <h3 style={{ margin: 6, marginBottom: 16 }}>Agency Placement Requests</h3>
...
Index.getInitialProps = async ({ req }) =>
{
    ...
    if (token)
    {
        formList = await adminRetrieveAllPlacementRequests(token);
    }
    if (!formList) {
        formList = [];
    }
    return { user, token, formList };
}